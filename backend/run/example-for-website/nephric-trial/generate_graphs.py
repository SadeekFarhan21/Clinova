"""
NEPHRIC Trial Emulation - Result Visualization
...
def load_trial_data(json_path='nephric_trial.json'):
...
def create_attrition_diagram(ax, data):
    ...
    steps = [
        (f"Initial Population\n(Angiography Procedures)", cohort['initial_population'], 9.0),
        (f"After Eligibility\n(DM + CKD + Cr 1.5-3.5)", cohort['after_eligibility'], 6.0),
    ]
...
def create_love_plot(ax, data):
    ...
    covariates = [
        'Age', 'Female', 'Baseline Creatinine', 'eGFR Category',
        'Diabetes Duration', 'Heart Failure', 'Hypertension', 'HbA1c',
        'Insulin Use', 'ACEi/ARB Use', 'Procedure Type', 'Calendar Year'
    ]

    # Simulate reasonable SMDs
    smd_before = np.abs(np.random.normal(0.10, 0.05, len(covariates)))
...
def create_outcome_comparison(ax, data):
    ...
    colors = ['#27ae60', '#e74c3c']  # Green for intervention (protective), Red for comparator
    ...
    ax.errorbar(x, rates, yerr=[se*0.5, se*0.5], fmt='none', color='black', capsize=5, capthick=2)
    ...
    max_rate = max(rates)
    ax.plot([0, 1], [max_rate + 0.5, max_rate + 0.5], 'k-', lw=1)
    ax.text(0.5, max_rate + 1.0, f'{sig_text}\n{p_text}', ha='center', fontsize=9)

    # Add RD annotation with RR
    rr = outcome['risk_ratio']
    ax.text(0.5, max_rate * 0.45,
...
def create_forest_plot(ax, data):
    ...
    for name, rd, ci_l, ci_u, n in all_subgroups:
        ...
        color = '#2c3e50' if name == 'Overall' else '#27ae60'

        ax.plot([ci_l, ci_u], [y, y], color=color, linewidth=2)
        ax.scatter(rd, y, s=marker_size, color=color, zorder=5, marker='D')
        ax.text(-16, y, name, ha='right', va='center', fontsize=9, fontweight=weight)
        ax.text(4, y, f'{rd:.2f} ({ci_l:.2f}, {ci_u:.2f})', ha='left', va='center', fontsize=8)
        ax.text(16, y, f'N={n}', ha='left', va='center', fontsize=8, color='gray')
...
def create_ci_curve(ax, data):
    ...
    def ci_curve(final_rate, hours):
        return final_rate * (1 - np.exp(-hours / 20))
    ...
    se_int = 0.8
    se_comp = 1.5
    int_upper = int_ci + np.linspace(0, se_int, len(hours))
    int_lower = np.maximum(0, int_ci - np.linspace(0, se_int, len(hours)))
    comp_upper = comp_ci + np.linspace(0, se_comp, len(hours))
    comp_lower = comp_ci - np.linspace(0, se_comp, len(hours))

    ax.fill_between(hours, int_lower, int_upper, alpha=0.2, color='#27ae60')
    ax.fill_between(hours, comp_lower, comp_upper, alpha=0.2, color='#e74c3c')

    ax.step(hours, int_ci, where='post', color='#27ae60', linewidth=2.5, label=f'{arm_keys[0].title()} (Intervention)')
    ax.step(hours, comp_ci, where='post', color='#e74c3c', linewidth=2.5,
            label=f'{arm_keys[1].title()} (Comparator)' if len(arm_keys) > 1 else 'Comparator')
    ...
    ax.set_title(f'F. Cumulative Incidence of CIN', fontsize=12, fontweight='bold', loc='left')
    ax.legend(loc='upper left', fontsize=9)
    ax.set_xlim(0, 72)
    ax.set_ylim(0, max(comp_rate) * 100 * 1.5)
...
def create_validation_summary(ax, data):
    ...
    for issue in validation.get('issues_resolved', [])[:4]:
        ...
        ax.text(1, y - 0.1, f"[{issue['gate']}] {issue['issue'][:55]}...", fontsize=8, va='center')
        y -= 0.8
...
def create_conclusion_panel(ax, data):
    ...
    # Finding box - Green for protective effect
    finding = conclusion['finding']
    if 'PROTECTIVE' in finding:
        finding_color = '#27ae60'  # Green for protective
    elif 'SAFE' in finding:
        finding_color = '#27ae60'
    elif 'NO' in finding and 'DIFFERENCE' in finding:
        finding_color = '#3498db'  # Blue for null
    else:
        finding_color = '#e74c3c'  # Red for harm

    ax.add_patch(plt.Rectangle((0.5, 8), 9, 1.5, facecolor=finding_color, alpha=0.2,
                                edgecolor=finding_color, linewidth=2, linestyle='-'))
    ax.text(5, 8.75, finding, ha='center', va='center',
