...
| Time zero | Contrast administration datetime | `drug_exposure_start_datetime` (or best-available proxy) |
...
> **Notation:** `t0` = time zero (contrast administration datetime).
...
| I2 | Contrast-enhanced MDCT performed at index | Include | `procedure_occurrence` contains CT w/ IV contrast **AND** contrast agent record (I3/I4) temporally linked | Procedure time within e.g. `t0 ± 6h` (linking rule) | `[OMOP: contrast-enhanced CT/MDCT procedure concepts]` | local / Athena versioned concept set |
| I3 | Received **iodixanol** IV for the index scan (intervention arm) **OR** | Include | `drug_exposure` (preferred): `drug_concept_id` for iodixanol; route = IV (if available); `drug_exposure_start_datetime = t0` | At `t0` | `[OMOP: iodixanol]` | RxNorm / local mapping snapshot |
| I4 | Received **iopamidol** IV for the index scan (comparator arm) | Include | `drug_exposure`: iopamidol | At `t0` | `[OMOP: iopamidol]` | RxNorm / local mapping snapshot |
| I5 | Baseline serum creatinine available pre-contrast | Include | `measurement` creatinine; choose “most recent qualifying” value before `t0` | **Primary:** within 24h pre-`t0` if inpatient/ED; within 7d pre-`t0` if outpatient (visit-type stratified rule) | `[OMOP: serum creatinine measurement]` | LOINC/OMOP Measurement |
...
| E5 | Multiple iodinated contrast agents at index (mixed exposure) | Exclude | Two+ distinct iodinated contrast `drug_concept_id`s within [-1h,+6h] of `t0` | Index day | `[OMOP: iodinated contrast media]` | versioned |
| E6 | Intra-arterial contrast procedure at index day (route-mixing) | Exclude | `procedure_occurrence` angiography/cath with contrast same day | 0–1d around `t0` | `[OMOP: angiography with contrast]` | versioned |
...
|---|---|---|---:|---|---|---|
| A (Intervention) | **Iodixanol** IV administered for index MDCT | `drug_exposure` contains iodixanol with start datetime = `t0` linked to MDCT procedure | None | Single administration at `t0` | Usual care after scan allowed (IV fluids, meds) | Exclude if mixed iodinated agents at index (E5) |
| B (Comparator) | **Iopamidol** IV administered for index MDCT | `drug_exposure` contains iopamidol with start datetime = `t0` linked to MDCT | None | Single administration at `t0` | Same | Same |
...
- Require an MDCT-with-contrast procedure record within **±6 hours** of contrast `drug_exposure_start_datetime`.
- If only procedure timestamp exists (no drug timestamp), define `t0 = procedure_datetime` and require the contrast drug exposure on the same calendar day within the same visit.
...
**Time zero (`t0`) = date/time of IV iodinated contrast administration for the index MDCT**, defined as:
1) Primary: `drug_exposure.drug_exposure_start_datetime` for iodixanol or iopamidol; else  
2) Fallback: `procedure_occurrence.procedure_datetime` for contrast-enhanced MDCT if drug datetime missing.

### 5.2 Three-way coincidence verification (HARD validity gate)
You must be able to assert (programmatically) that:
- Eligibility is assessed using only data **≤ t0** (baseline creatinine strictly pre-`t0`).
- Treatment assignment (iodixanol vs iopamidol) is determined **at t0**.
- Outcome counting begins **after t0** (post-contrast creatinine measurements strictly after `t0`).
