...
| Time zero | Contrast administration datetime | **Time zero (`t0`) = contrast `drug_exposure_start_datetime` (or start date in date-only mode) for the linked scan episode (see §5)** |
...
> **Notation:** `t0` = time zero (contrast administration datetime/date for the scan episode; see §5).
...
| I3 | Received **iodixanol** IV for the index scan (intervention arm) **OR** | Include | `drug_exposure` (preferred): `drug_concept_id` for iodixanol; route = IV (if available); exposure record must be part of the linked scan episode used to define `t0` | At `t0` (assignment at contrast start) | `[OMOP: iodixanol]` | RxNorm / local mapping snapshot |
| I4 | Received **iopamidol** IV for the index scan (comparator arm) | Include | `drug_exposure`: iopamidol; part of linked scan episode | At `t0` (assignment at contrast start) | `[OMOP: iopamidol]` | RxNorm / local mapping snapshot |
...
- **Date-only tie-break rule (when datetimes are unavailable for `t0` and/or creatinine):**
  - Define `t0_date = DATE(t0)` (from the contrast administration date in the scan episode).
...
| B (Comparator) | **Iopamidol** IV administered for index MDCT | `drug_exposure` contains iopamidol as part of linked scan episode | At `t0` | Single administration at `t0` | Same | Same |
...
- **Temporal proximity rule (datetime-aware):**
  ...
  - **Define `t0` as the contrast administration time:** `t0 = drug_exposure_start_datetime`.  
    The CT `procedure_datetime` is used **only for linkage/confirmation** of the scan episode and **does not shift `t0` later**.
...
- **If only dates are available (date-only mode):**
  ...
  - Define `t0_date = drug_exposure_start_date` (the contrast administration date) and use `t0_date` for windowing, using the tie-break rules in §4.1 and §5.2.
...
**Time zero (`t0`) = the contrast administration datetime for the linked MDCT scan episode**, defined so that **treatment assignment time equals follow-up start**:

1) Identify a linked **scan episode** consisting of:
   - an iodixanol or iopamidol `drug_exposure` record (contrast administration), and
   - an MDCT-with-IV-contrast `procedure_occurrence` record,
   within the same visit and within the temporal proximity rule in §2.2.

2) **Define `t0` as:**
   - **Primary (datetime-aware):** `t0 = drug_exposure_start_datetime` for the iodixanol/iopamidol record in the scan episode.  
     The CT `procedure_datetime` is used for linkage only and **does not define or delay `t0`**, even if it is recorded after the contrast time.
   - **Date-only mode:** if only dates are available, define `t0_date = drug_exposure_start_date` (and require the CT `procedure_date` matches it per §2.2) and use `t0_date` for windowing (see §4.1 and §5.2).

### 5.2 Three-way coincidence verification (HARD validity gate)
You must be able to assert (programmatically) that:
- All **clinical baseline eligibility criteria** (e.g., CKD labs, exclusions) are assessed using only data **≤ t0** (baseline creatinine strictly pre-`t0` under the applicable datetime/date-only rule).
- Treatment assignment (iodixanol vs iopamidol) is determined **at t0** (by the contrast `drug_exposure_start_datetime`/date).
...
> **Note on CT procedure timestamps:** CT `procedure_datetime` can reflect documentation/completion time and may occur after contrast start. Therefore, for alignment validation we (i) anchor `t0` on contrast administration, and (ii) require the CT procedure record to be linked within ±6h of `t0` in the same visit, without allowing it to shift `t0` later.

**Machine-check rules (datetime-aware):**
- `t0 == drug_exposure_start_datetime` (assignment time equals follow-up start)
- `Cr0_time < t0`
- all post-contrast creatinine times used for `Crmax72` satisfy `t0 < Cr_time ≤ t0+72h`
- scan episode linkage: `abs(procedure_datetime - t0) ≤ 6h` and same visit (preferred)

**Machine-check rules (date-only mode):**
- Define `t0_date = drug_exposure_start_date` (contrast administration date)
- `Cr0_date < t0_date`
- all post-contrast creatinine dates used satisfy `t0_date < Cr_date ≤ t0_date + 3 days`
- linkage requires `procedure_date == t0_date` (same visit preferred)
...
|---|---|---|---|
| Index MDCT (scan confirmation) | `procedure_occurrence` | `procedure_concept_id`, `procedure_datetime`, `visit_occurrence_id` | Used to confirm CT context and link to contrast within ±6h; **does not define `t0`** |
| Contrast agent (assignment + time zero) | `drug_exposure` | `drug_concept_id`, `drug_exposure_start_datetime`, `route_concept_id`, `dose_unit_concept_id`, `quantity` | **Defines `t0`** as `drug_exposure_start_datetime` (or start date in date-only mode) |
